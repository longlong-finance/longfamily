...
describe("Yearn V1 Vault USDC test", function(){
  ...
  let yVaultUSDC = "0x597aD1e0c13Bfe8025993D9e79C69E1c0233522e";

  // settings
  let baseERC20;

  before(async function() {
    await resetToBlock(12304074);
    ...
    yearnV1IVImplementation= await YearnV1VaultV1Base.new();
    ...
    }, [store.address, baseERC20.address, yVaultUSDC]);

    let ivProxy = await TimelockProxyStorageCentered.new(
      yearnV1IVImplementation.address,
      store.address,
      0,    // no timelock
      yearnIVInitCall,
      {from: admin}
    ...
    ivYearn = await YearnV1VaultV1Base.at(ivProxy.address);
  ...
  describe("YearnIV", function() {
    ...
    let yearnControllerAddr = "0x9e65ad11b299ca0abefc2799ddb6314ef2d91080";
    let yearnController;
    let yearnStrategy;
    let yearn3CRVStrategist = "0x02027bda2425204f152b8aa35fb78687d65e1af5";
    ...
    before(async function() {
      ...
      yearnController = await IYearnController.at(yearnControllerAddr);
      yearnStrategyAddr = await yearnController.strategies(usdc);
    ...
    beforeEach(async function() {
      ...
      await obtainAssetFor(user, usdcHolder, usdc, "100000" + "000000");
    });
    ...
    it("Deposit and Withdraw immediately", async function(){
      ...
      await vault.withdraw(depositHalfBalance, {from: user});
      assert.approximately((
        (new BN(depositHalfBalance)).sub(await baseERC20.balanceOf(user))).toNumber(),
        0, 10
      );
    });
    ...
    it("Deposit and Withdraw after yearn harvest", async function(){
      ...
      // Yearn Harvest
      y3CRVStrategy = await IYearnStrategy.at("0xC59601F0CC49baa266891b7fc63d2D5FE097A79D");
      await y3CRVStrategy.harvest({from: yearnGov})

      yVault = await IYearnVault.at(yVaultUSDC);
      console.log( (await yVault.getPricePerFullShare()).toString());
      await yearnStrategy.harvest({from: yearnGov});
      console.log( (await yVault.getPricePerFullShare()).toString());
